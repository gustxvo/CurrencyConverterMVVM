...
import com.gustxvo.currencyconverter.data.model.Rates
        ...
        viewModelScope.launch(dispatchers.io) {
            ...
            when (val ratesResponse = repository.getRates(fromCurrency)) {
                ...
                is Resource.Success -> {
                    ...
                    if (rate == null) {
                        _conversion.value = CurrencyEvent.Failure("An Unexpected value occurred")
                    } else {
                        val convertedCurrency = round(fromAmount * rate * 100) / 100
                        _conversion.value = CurrencyEvent.Success(
                            "$fromAmount $fromCurrency = $convertedCurrency to $toCurrency"
                        )
                    }
                }
            }
